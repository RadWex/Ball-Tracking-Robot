...
class JetsonVideoStream:
    def __init__(self, captureResolution=(3264, 1848), outputResolution=(960, 460), frameRate=28, flipMethod=0,
        ...
        if exposureTimeInMiliseconds is not None:
            ...
            exposureTimeString = 'exposuretimerange="%d %d" ' % (
        ...
        whiteBalanceModeString = 'wbmode=%d ' % whiteBalanceMode if whiteBalanceMode is not 1 else ''
    ...
    def update(self):
        ...
        while True:
            ...
            if self.stopped:
                # TODO: only if it is necessary to release after stopping, can prevent resource blocking
                self.stream.release()
                return
